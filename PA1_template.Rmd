...
- Mean total steps taken per day : `r format(round(st.mean,0),scientific=NA)`
- Median total steps taken per day: `r format(round(st.median,0),scientific=NA)`
...
```
The maximum number of steps per interval is `r round(m1$steps,0)` and corresponds to the interval `r m1$interval`. 
...
```
There are `r sum(mv)` rows with missing values.
...
```
- New Mean total steps taken per day : `r format(round(st.mean2,0),scientific=NA)`
- New Median total steps taken per day: `r format(round(st.median2,0),scientific=NA)`
